...
public class ReceiveMessageHandler {
    ...
    private void forward_message_to_worker(Message m, String host, int worker, int logic) {
        int worker_port = 8500 + worker;
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(Collections.singletonList(MediaType.APPLICATION_JSON));
        HttpEntity<Message> entity = new HttpEntity<>(m, headers);
        ResponseEntity<?> response = restTemplate.exchange("http://" + host + ":" + worker_port + "/message", HttpMethod.POST, entity, String.class);
    ...
    public void process_message_architecture_3(Message message) {
        ...
        forward_message_to_worker(message, "localhost", worker_to_forward, 3);
    }
    ...
    public void process_message_architecture_4(Message message) {
        ...
        message.set_worker(worker_to_forward);
        forward_message_to_worker(message, "localhost", worker_to_forward, 4);
    }
