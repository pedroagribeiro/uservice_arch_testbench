...
public class Worker {
    ...
    public final static Map<String, String> endpoints = new HashMap<>() {{
        ...
        put("target", "http://" + Worker.host + ":" + Worker.port + "/run/target?target={target}");
    }}; 
    ...
    public static void forward_orchestration(Orchestration orchestration, int worker) {
        ...
        if(response.getStatusCode().isError()) {
            log.error(String.format("The orchestration could not be forwarded to the worker %d. Something went wrong!", worker));
        } else {
            if(response.getStatusCode().is2xxSuccessful()) {
                log.info(String.format("Informed the worker %d of the orchestration!", worker));
            }
        }
    }
    ...
    public static void inform_of_target(int target, int worker) {
        ...
        if(response.getStatusCode().isError()) {
            log.info(String.format("Could not inform the worker %d of the run target. Something went wrong!", worker));
        } else {
            if(response.getStatusCode().is2xxSuccessful()) {
                log.info(String.format("Informed the worker %d of the run target!", worker));
            }
        }
    }
}
